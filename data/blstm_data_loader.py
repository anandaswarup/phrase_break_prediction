...
class PhraseBreakDataset(Dataset):
    ...
    def __init__(self, data_dir, split="train"):
        ...
        self.sentences = read_dataset_file(
            os.path.join(data_dir, f"{split}/sentences.txt")
        )
        self.punctuations = read_dataset_file(
            os.path.join(data_dir, f"{split}/punctuations.txt")
        )
    ...
    def __getitem__(self, index):
        ...
        text_seq = [
            self.word_vocab[word] if word in self.word_vocab else self.word_unk_idx
    ...
    def pad_collate(self, batch):
        ...
        padded_sentences = pad_sequence(
            sentences, batch_first=True, padding_value=self.word_pad_idx
        )
        padded_punctuations = pad_sequence(
            punctuations, batch_first=True, padding_value=-1
        )
